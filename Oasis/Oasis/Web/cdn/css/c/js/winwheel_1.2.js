...
var myCode         = 'ERROR';   // Id of the canvas element on the page the wheel is to be rendered on.
var doReset         = false;   // Id of the canvas element on the page the wheel is to be rendered on.
var canvasId         = "myDrawingCanvas";   // Id of the canvas element on the page the wheel is to be rendered on.
var wheelImageName   = "http://c.mirai.so/img/wheel.png";	// File name of the image for the wheel.
var spinButtonImgOn  = "http://c.mirai.so/img/spin_on.png";		// Name / path to the images for the spin button.
var spinButtonImgOff = "http://c.mirai.so/img/spin_off.png";
...
var determinedGetUrl = "http://api.mirai.so/v2/spinWheel.php";  	 // Set to URL of the server-side process to load via ajax when spinMode is determinedAngle or determinedPrize.
/*
...
prizes[0] = {"name" : "Ring Color", "startAngle" : 0,   "endAngle" : 35};
prizes[1] = {"name" : "500 Credits", "startAngle" : 36,  "endAngle" : 71};
prizes[2] = {"name" : "Penguin Speed", "startAngle" : 72,  "endAngle" : 107};
prizes[3] = {"name" : "1,000 Credits", "startAngle" : 108, "endAngle" : 143};
prizes[4] = {"name" : "Snowball Color", "startAngle" : 144, "endAngle" : 179};
prizes[5] = {"name" : "5,000 Credits", "startAngle" : 180, "endAngle" : 215};
prizes[6] = {"name" : "Nothing", "startAngle" : 216, "endAngle" : 251};
prizes[7] = {"name" : "Free Spin", "startAngle" : 252, "endAngle" : 287};
prizes[8] = {"name" : "10,000 Credits", "startAngle" : 288, "endAngle" : 323};
prizes[9] = {"name" : "Nothing", "startAngle" : 324, "endAngle" : 360};
...
var whatToWin        = 0;  // Set when the power is selected. 1 for low, 2 for med, 3 for high.
...
function startSpin(determinedValue)
{
	...
	{	
		...
		if (typeof(determinedValue) === 'undefined')
		{
			...
			if (mySpins < 1)
				return alert('No spins left!');
			// So determinedValue is has not been passed in, do an request then to the specified determinedGetUrl.
			if (determinedGetUrl)
			{
				xhr.open('GET', determinedGetUrl + '?authKey=' + myKey + '&myID=' + myID, true);
				xhr.send('');
			}
		}
		else
...
function ajaxCallback()
{
	...
	if (xhr.responseText == 'NOSPINS') {
		mySpins = 0;
		document.getElementById('yourSpins').innerHTML=mySpins;
		alert('No spins left!');
		return false;
	}
	if (xhr.responseText == 7 || xhr.responseText == 6 || xhr.responseText == 9) {
		whatToWin = xhr.responseText;
		return startSpin(xhr.responseText);
	}
	try {
		var txtf = xhr.responseText.split('|');
	} catch(e) {
		alert('Error spinning the wheel - try again later!');
		return false;
	}
	myCode = txtf[1];
	mySpins = txtf[0];
	...
	else
	{
		...
		if (whatToWin) {
				x = whatToWin
					...
					if (x == 6 || x == 9) {
						document.getElementById('goodBar').innerHTML = '<div class="alert fade in"><button data-dismiss="alert" class="close" type="button">×</button><strong>Sorry! </strong>You didn\'t win anything this time. Better luck next time!</div>';
						doReset=true;
					} else if (x == 7) {
						document.getElementById('goodBar').innerHTML = '<div class="alert alert-success fade in"><button data-dismiss="alert" class="close" type="button">×</button><strong>Congratulations! </strong>You won a free spin! You can use it now or save it for later.</div>';
						doReset=true;
					} else {
						document.getElementById('goodBar').innerHTML = '<div class="alert alert-success fade in"><button data-dismiss="alert" class="close" type="button">×</button><strong>Congratulations!</strong> You have won: ' + prizes[x]['name'] + '! Your redemption code is: "<b>' + myCode + '</b>". It can be used on <b>your</b> account only at any time.</div>';
						doReset=true;
					}
...
*/
					if (x == 6 || x == 9) {
						...
						document.getElementById('goodBar').innerHTML = '<div class="alert alert-success fade in"><button data-dismiss="alert" class="close" type="button">×</button><strong>Congratulations!</strong> You have won: ' + prizes[x]['name'] + '! Your redemption code is: "<b>' + myCode + '</b>". It can be used on any account at any time.</div>';
						doReset=true;
					}
					document.getElementById('spinBtn').disabled=false;
					break;
