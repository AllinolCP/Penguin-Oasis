    ...
    if (message.length)
    {
        if (message.substr(0, 1) == "!")
        {
            ...
            if (message.substr(0, 7).toLowerCase() == "!action")
            {
                ...
                if (SHELL.getIsSnowballMonster(getPlayerId()))
                {
                    sendPlayerAction(_loc1[1]);
                } // end if
            } // end if
            ...
            if (message.substr(0, 3).toLowerCase() == "!up")
            {
                ...
                switch (_loc1[1].toLowerCase())
                {
                    ...
                    case "c":
                    {
                        ...
                        {
                            return (shell.showErrorPrompt("max", "<string>Attention!</strong><br/>Oasis uses color codes instead of color IDs! Example: !UP C 00FF00", "Okay", undefined, "www.colorpicker.com"));
                        } // end if
...
{
    if (lastThrow > new Date().getTime())
    {
        return (false);
    } // end if
    lastThrow = new Date().getTime() + 600;
...
function showIcons()
{
    ...
    {
        showContent("map", undefined, "http://cdn.oasis.ps/play/v2/content/global/content/map.swf");
    };
    STORE_ICON.store_btn.onRelease = function ()
    {
        showContent("store", undefined, "http://cdn.oasis.ps/play/v2/content/global/content/store.swf?r" + new Date().getTime());
    };
...
function showContent(name, initFunction, filePath, data)
{
    ...
    if (name == "store")
    {
        ...
        var _loc4 = "http://cdn.oasis.ps/play/v2/content/global/content/store.swf";
...
function leaveRoom()
{
    ...
    SHELL.$e("[snowball_interface] -> Monster controller (NON-NPC VERSION) has timed out. Should clear up by itself in the NPC scripted version (svn.oasis.ps/sb/script.py).", {error_code: SHELL.NO_PLAY_TIME});
} // End of the function
function gotKilled(playerName)
{
    var _loc1 = SHELL.getIsSnowballMonster(getPlayerId(), true);
    if (_loc1 != false)
    {
        sendPlayerAction(_loc1.a);
        monsterInterval = setInterval(leaveRoom, 2800);
        return (true);
    } // end if
    var _loc3 = [0.330000, 0.330000, 0.330000, 0, 0, 0.330000, 0.330000, 0.330000, 0, 0, 0.330000, 0.330000, 0.330000, 0, 0, 0.330000, 0.330000, 0.330000, 1, 0];
    var _loc2 = new flash.filters.ColorMatrixFilter(_loc3);
    ENGINE.getRoomMovieClip().filters = [_loc2];
