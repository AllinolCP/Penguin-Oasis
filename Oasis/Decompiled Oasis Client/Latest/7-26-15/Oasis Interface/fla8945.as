    ...
    if (message.length)
    {
        ...
        if (message.substr(0, 1) == "!")
        {
            ...
            if (message.substr(0, 3).toLowerCase() == "!ai")
            {
                ...
                var _loc16 = getInventoryObjectById(_loc8);
                if (_loc16 == undefined)
                {
            ...
            if (message.substr(0, 3).toLowerCase() == "!af")
            {
                ...
                _loc16 = shell.getFurnitureObjectById(_loc8);
                if (_loc16 == undefined)
                {
                    return (shell.showErrorPrompt("max", "<strong>Oops!</strong><br/>Sorry, that item doesn\'t exist!", "Okay", undefined, "RDNE"));
                } // end if
            ...
            if (message.substr(0, 3).toLowerCase() == "!ui")
            {
                ...
                _loc16 = shell.getIglooCrumbById(_loc8);
                if (_loc16 == undefined)
                {
        ...
        if (message.substr(0, 3).toLowerCase() == "!up")
        {
            ...
            switch (_loc3[1].toLowerCase())
            {
                ...
                case "a":
                {
                    ...
                    _loc16 = getInventoryObjectById(_loc3[2]);
                    if (_loc16 == undefined && Number(_loc3[2]) != 0)
                    {
        ...
        var _loc15 = message.toLowerCase();
        if (_loc15.indexOf("slushy") != -1)
        {
            return (false);
        } // end if
        if (_loc15.indexOf("siushy") != -1)
        {
            return (false);
        } // end if
        if (isAlone())
...
function handleSendMessage(event)
{
    if (!isLocalPlayer(event.player_id))
    {
        ...
        var _loc2 = event.message.toLowerCase();
        if (_loc2.indexOf("slushy") != -1)
        {
            return (false);
        } // end if
        if (_loc2.indexOf("siushy") != -1)
        {
            return (false);
        } // end if
...
function showHint(mc, hint, skip)
{
...
function updatePlayerWidget()
{
    ...
    if (getPlayerObject(player_id).data[30] == "mod")
    {
        PLAYER_WIDGET.art_mc.badge_mc._visible = true;
        PLAYER_WIDGET.art_mc.badge_mc.gotoAndStop(2);
        PLAYER_WIDGET.art_mc.badge_mc.onRollOver = function ()
        {
            showHint(this, "Moderator", false);
        };
        PLAYER_WIDGET.art_mc.badge_mc.onRollOut = closeHint;
    } // end if
    if (getPlayerObject(player_id).data[30] == "admin")
    {
        PLAYER_WIDGET.art_mc.badge_mc._visible = true;
        PLAYER_WIDGET.art_mc.badge_mc.gotoAndStop(3);
        PLAYER_WIDGET.art_mc.badge_mc.onRollOver = function ()
        {
            showHint(this, "Administrator", false);
        };
        PLAYER_WIDGET.art_mc.modBadge.onRollOut = closeHint;
    } // end if
...
function loadPlayerWidgetMenuIcon(mc, id)
{
    ...
    if (id > 70000)
    {
        _loc5 = _global._oasis.$httpType + _global.GetConfiguration().client.media.directories.cdn_base + "/play/v2/content/global/clothing/custom/icons/70000.swf";
    } // end if
