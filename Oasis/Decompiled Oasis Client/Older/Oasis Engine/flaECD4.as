...
function handlePlayerMove(player_ob)
{
    ...
    if (!SHELL.isMyPlayer(_loc1))
    {
        ...
        var _loc2 = findPlayerPath(_loc1, _loc4, _loc5);
        var _loc7 = _loc2.x;
        var _loc6 = _loc2.y;
...
{
    var _loc12 = getRoomMovieClip();
    var _loc21 = getRoomBlockMovieClip();
    var _loc3 = player_ob.player_id;
    var _loc10 = player_ob.nickname;
    removePlayer(_loc3);
    var _loc11 = addPlayerDepth(_loc3);
    var _loc9 = "p" + String(_loc3);
    var _loc4 = INTERFACE.nicknames_mc.attachMovie("nickname", _loc9, _loc11, {_x: targetX, _y: targetY, _visible: false});
    var _loc7 = INTERFACE.nicknames_mc[_loc9].name_txt;
    _loc7.text = _loc10;
    _loc7.textColor = 0;
    _loc12.createEmptyMovieClip(_loc9, _loc11);
    var _loc2 = _loc12[_loc9];
    ...
    _loc2.nickname = _loc10;
    _loc2.depth_id = _loc11;
    _loc2.frame = player_ob.frame;
    _loc2.is_moving = false;
    _loc2.is_ready = false;
    _loc2.is_reading = false;
    _loc2.is_table = false;
    _loc2.account_permissions = player_ob.account_permissions;
    _loc2.char_permissions = player_ob.char_permissions;
    _loc2.player_attributes = player_ob.player_attributes;
    if (!SHELL.isLag)
    {
        if (_loc2.player_attributes.x != 0)
        {
            switch (_loc2.player_attributes.x)
            {
                case 1:
                {
                    _loc2._xscale = 60;
                    ...
                    _loc5.size = 8;
                    _loc4.name_txt.setTextFormat(_loc5);
        ...
        if (_loc2.player_attributes.n[1] != "0" && _loc2.player_attributes.n[1] != "" && _loc2.player_attributes.n[1] != undefined)
        {
            _loc7.textColor = "0x" + _loc2.player_attributes.n[1];
        } // end if
        var _loc15 = String("0x") + String(_loc2.player_attributes.n[0]);
        if (_loc2.player_attributes.n[0] != "0" && _loc2.player_attributes.n[0] != "" && _loc2.player_attributes.n[0] != undefined)
        {
            _loc7.filters = [new flash.filters.GlowFilter(_loc15, 10, 1.700000, 1.700000, 15, 3, false, false)];
        } // end if
    } // end if
    _loc2.onRelease = com.clubpenguin.util.Delegate.create(this, clickPlayer, _loc3, _loc10);
    if (SHELL.isMyPlayer(_loc3))
    {
        var _loc16 = SHELL.getLocalizedString("load_penguin");
        SHELL.showLoading(_loc16, listener);
    } // end if
    var _loc13 = com.clubpenguin.util.URLUtils.getCacheResetURL(SHELL.getPath("penguin"));
    var _loc8 = new com.clubpenguin.hybrid.HybridMovieClipLoader();
    _loc8.addEventListener(com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_INIT, com.clubpenguin.util.Delegate.create(this, onPlayerLoadInit, player_ob, _loc3, targetX, targetY));
    _loc8.addEventListener(com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_START, com.clubpenguin.util.Delegate.create(this, onPlayerLoadStart, target_mc));
    _loc8.addEventListener(com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_PROGRESS, com.clubpenguin.util.Delegate.create(this, onPlayerLoadProgress, target_mc));
    _loc8.loadClip(_loc13, _loc2.art_mc);
    _loc2.art_mc.ring._visible = false;
    if (!SHELL.isLag)
    {
        if (_loc2.player_attributes.r != undefined && _loc2.player_attributes.r != "")
        {
            _loc2.art_mc.ring._visible = true;
            var _loc14 = new Color(_loc2.art_mc.ring);
            _loc14.setRGB("0x" + _loc2.player_attributes.r);
        }
        else
        {
            _loc2.art_mc.ring._visible = false;
...
function onPlayerLoadInit(event, player_ob, player_id, targetX, targetY)
{
    ...
    else if (isPositionValid(player_ob.x, player_ob.y) || _global.getCurrentShell().OasisPermission.hasPermission("char", player_ob.char_permissions, "wall_hack") == true)
    {
...
function updatePlayer(player_ob)
{
    ...
    if (!SHELL.isLag)
    {
        if (_loc1.player_attributes.r != undefined && _loc1.player_attributes.r != "")
        {
...
function loadPlayerItem(mc, item_id)
{
    if (item_id > 0)
    {
        ...
        mc._visible = false;
        var _loc2 = new com.clubpenguin.hybrid.HybridMovieClipLoader();
        _loc2.addEventListener(com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_COMPLETE, mx.utils.Delegate.create(this, onPlayerItemLoadComplete));
        _loc2.addEventListener(com.clubpenguin.hybrid.HybridMovieClipLoader.EVENT_ON_LOAD_INIT, mx.utils.Delegate.create(this, onPlayerItemLoadInit));
...
{
    var _loc8 = getPlayerMovieClip(player_id);
    var _loc9 = getRoomBlockMovieClip();
    var _loc15 = getValidXPosition(x);
    var _loc14 = getValidYPosition(y);
    var _loc13 = Math.round(_loc8._x);
    var _loc12 = Math.round(_loc8._y);
    var _loc16 = findDistance(_loc13, _loc12, _loc15, _loc14);
    var _loc7 = Math.round(_loc16);
    var _loc10 = (_loc15 - _loc13) / _loc7;
    var _loc11 = (_loc14 - _loc12) / _loc7;
    var _loc5 = _loc13;
    var _loc6 = _loc12;
    var _loc4 = new Object();
    _loc4.x = _loc13;
    _loc4.y = _loc12;
    var _loc17 = _loc9.hitTest(_loc13, _loc12, true);
    if (_global.getCurrentShell().OasisPermission.hasPermission("char", _loc8.char_permissions, "wall_hack") == true)
    {
        _loc17 = 0;
    } // end if
    while (_loc7 > 0)
    {
        _loc5 = _loc5 + _loc10;
        _loc6 = _loc6 + _loc11;
        var _loc2 = Math.round(_loc5);
        var _loc3 = Math.round(_loc6);
        if (_loc9.hitTest(_loc2, _loc3, true))
        {
            if (_global.getCurrentShell().OasisPermission.hasPermission("char", _loc8.char_permissions, "wall_hack") == true)
            {
                _loc4.x = _loc2;
...
function throwBallOverride(player_id, target_x, target_y, start_height, max_height, wait)
{
    ...
    if (_loc2.is_ready && !_loc2.is_moving)
    {
        ...
        if (_loc2.player_attributes.sc != "" && !SHELL.isLag)
        {
            if (_loc2.player_attributes.sc.length == 6)
            {
                var _loc4 = new Color(mc);
                _loc4.setRGB("0x" + _loc2.player_attributes.sc);
            } // end if
        } // end if
        ...
        var _loc5 = findDistance(start_x, start_y, target_x, target_y);
        var _loc6 = findAngle(start_x, start_y, target_x, target_y);
        var _loc7 = Math.round(findDirection(_loc6) / 2);
        updatePlayerFrame(player_id, 26 + _loc7);
        var duration = _loc5 / 15;
        var change_x = target_x - start_x;
        var change_y = target_y - start_y;
        var peak = duration / 2;
