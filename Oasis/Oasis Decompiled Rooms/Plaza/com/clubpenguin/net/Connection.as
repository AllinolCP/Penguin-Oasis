class com.clubpenguin.net.Connection extends XMLSocket implements com.clubpenguin.net.IConnection
{
    ...
    function sendXtMessage(xtName, cmdName, paramObj, type, roomId)
    {
        ...
        var _loc4 = cmdName.split("#")[1];
        for (var _loc2 = 0; _loc2 < disabledCommands.length; ++_loc2)
        {
            if (disabledCommands[_loc2] == _loc4)
            {
                trace ("sendXTMessage command: " + _loc4 + " is disabled.");
                return;
            } // end if
        } // end of for
        if (roomId == undefined)
        ...
        {
            var _loc3;
            _loc3 = serverMessageDelimiter + "xt" + serverMessageDelimiter + xtName + serverMessageDelimiter + cmdName + serverMessageDelimiter + roomId + serverMessageDelimiter;
            for (var _loc2 = 0; _loc2 < paramObj.length; ++_loc2)
            {
                _loc3 = _loc3 + (paramObj[_loc2].toString() + serverMessageDelimiter);
            } // end of for
    ...
    function connectionEstablished(ok)
    {
        ...
        if (ok)
        {
            ...
            preConnection = false;
            var _loc3 = {t: "sys"};
            var _loc4 = "<ver v=\'" + majVersion.toString() + minVersion.toString() + subVersion.toString() + "\' />";
    ...
    function strReceived(message)
    {
        ...
        var _loc7 = _loc4[0];
        _responded.dispatch(_loc3, _loc6);
        messageHandlers[_loc7].handleMessage(_loc4.splice(1, _loc4.length - 1), this, "str");
    } // End of the function
