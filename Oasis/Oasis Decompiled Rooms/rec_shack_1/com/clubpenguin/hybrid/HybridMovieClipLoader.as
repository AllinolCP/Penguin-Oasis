class com.clubpenguin.hybrid.HybridMovieClipLoader extends mx.events.EventDispatcher
{
    ...
    function loadClip(url, container, callerName, isCDNCache)
    {
        ...
        else
        {
            ...
            {
                var _loc3 = this.searchURLforCacheVersion(url);
                _url = url + _loc3;
            }
            else
            {
    ...
    {
        var _loc2 = urlS.split("v2/", 2);
        if (_loc2 != null && _loc2.length > 1)
        {
            var _loc3 = _loc2[1].split("/", 2);
            switch (_loc3[0])
            {
                case "content":
                {
            ...
            trace ("INFO: Did not find client, content, games or config folder");
            return ("");
        } // end if
        trace ("ERROR: There was no v2 folder found when splitting up the url: " + urlS);
        return ("");
    } // End of the function
