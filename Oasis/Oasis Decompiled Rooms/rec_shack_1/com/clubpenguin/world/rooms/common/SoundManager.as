class com.clubpenguin.world.rooms.common.SoundManager
{
    ...
    function destroy()
    {
        for (var _loc4 in _soundLoopsMap)
        {
            var _loc3 = _soundLoopsMap[_loc4];
            this.stopSoundLoop(_loc3.loopId);
        } // end of for...in
        for (var _loc5 in _delayedSoundsMap)
        {
            var _loc2 = _delayedSoundsMap[_loc5];
            this.cancelDelayedSound(_loc2.delayedSoundId);
        } // end of for...in
        _frameCounter.removeMovieClip();
        _frameCounter = null;
    ...
    function playSound(symbolId, layerId, volume, tag, callback)
    {
        ...
        {
            var _loc8 = _registeredSymbolNamesMap[symbolId];
            if (_loc8 == null || _sfxHolder == null)
            {
                return (-1);
            } // end if
            var _loc7 = _sfxHolder.getNextHighestDepth();
            var _loc6 = _sfxHolder.createEmptyMovieClip("sfx_" + symbolId + "_" + _loc7, _loc7);
            var _loc3 = new Sound(_loc6);
            _loc3.attachSound(_loc8);
            if (_loc3.duration != undefined)
            {
                var _loc2 = new Object();
                var _loc5 = this.getSoundId();
                _loc2.soundId = _loc5;
                _loc2.tag = tag;
                _loc2.sfx = _loc3;
                _loc2.layerId = layerId;
                _loc2.targetHolder = _loc6;
                _loc2.callback = callback;
                _loc2.symbolId = symbolId;
                _loc2.layer = _loc4;
                _activeSoundsMap[_loc5] = _loc2;
                ++_loc4.numSoundsPlaying;
                _loc3.onSoundComplete = com.clubpenguin.util.Delegate.create(this, onSoundComplete, _loc3, _loc4, _loc6, tag, callback);
    ...
    function stopSoundsForTag(tag)
    {
        ...
        {
            var _loc2 = _activeSoundsMap[_loc6];
            if (_loc2.tag == tag)
            {
                _loc2.sfx.stop();
                this.onSoundComplete(_loc2.sfx, _loc2.layer, _loc2.targetHolder, _loc2.callback);
                delete _activeSoundsMap[_loc2.soundId];
            } // end if
        } // end of for...in
        for (var _loc5 in _soundLoopsMap)
        {
            var _loc3 = _soundLoopsMap[_loc5];
            if (_loc3.tag == tag)
            {
                this.stopSoundLoop(_loc3.loopId);
            } // end if
        } // end of for...in
        for (var _loc7 in _delayedSoundsMap)
        {
            _loc2 = _delayedSoundsMap[_loc7];
            if (_loc2.tag == tag)
            {
                this.cancelDelayedSound(_loc2.delayedSoundId);
            } // end if
